...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs, Eigen::VectorXd& mpc_x, Eigen::VectorXd& mpc_y) {
  bool ok = true;
  size_t i;
  ...
  // Fill up mpc points
  //cerr << "Filling up mpc points" << endl;
  for (unsigned int index=0; index < N; index++){
	  //cerr << index << ":" << solution.x[x_start + index] << endl;
	  //cerr << index << ":" << solution.x[y_start + index] << endl;
	  mpc_x(index) = solution.x[x_start + index];
	  mpc_y(index) = solution.x[y_start + index];
  }
